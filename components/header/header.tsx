...
export default function Header() {
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          'fixed top-0 left-0 bottom-0 z-50 w-28 bg-[#F1F5FA] border-r border-gray-200 shadow-2xl',
      ...
      >
        <div className="flex flex-col h-full">
            {/* 로고 - 클릭하면 홈으로 + 호버 이펙트 완벽 적용 */}
            <div 
                className="h-16 flex items-center justify-center border-b border-gray-200 px-4 cursor-pointer select-none group"
              ...
              {mainMenu.map((item) => {
                ...
                return (
                  <a
                    key={item.name}
                    href={item.href}
                    className="group flex flex-col items-center gap-2 px-2 py-3 rounded-2xl hover:bg-white/70 transition-all duration-200 hover:scale-105"
                    onClick={() => setIsOpen(false)}
                  >
                    <Icon36
                      name={item.icon}
