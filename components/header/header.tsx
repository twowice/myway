...
const isMenuPath = (pathname: string, href: string) =>
  href === '/' ? pathname === '/' || pathname.startsWith('/eventpage/') : pathname.startsWith(href);

// 반응형 수정: 모바일 하단 탭바에서는 긴 메뉴명을 짧게 보여줍니다.
const getMobileMenuName = (name: string) => {
  if (name === "파티 모집") return "파티";
  if (name === "마이페이지") return "마이";
  return name;
};
...
export default function Header() {
  ...
  // 페이지 이동 시 자동으로 패널 열기
  useEffect(() => {
    const current = mainmenu.find((item) => isMenuPath(pathname, item.href));
    if (current) {
    ...
    <>
      ...
      <aside className="hidden md:fixed md:inset-y-0 md:left-0 md:z-50 md:flex lg:w-16 bg-secondary border-r border-gray-200 shadow-2xl">
        <div className="flex flex-col h-full">
          {/* 로고 */}
          <div
            className="h-16 flex items-center justify-center border-b border-gray-200 px-2 cursor-pointer select-none group shrink-0"
            onClick={() => {
              router.push("/");
              router.refresh();
              ...
              {mainmenu.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      "w-full flex flex-col items-center gap-1.5 py-3 rounded-2xl transition-all duration-300 hover:scale-105 cursor-pointer",
                      isactive && "bg-blue-50"
                    )}
                  >
                    <Icon36
        ...
        {mainmenu.map((item) => {
          ...
          return (
            <button
              key={item.name}
              onClick={() => handlemenuclick(item.href, item.panelkey)}
              className="flex h-full flex-1 flex-col items-center justify-center gap-1 transition-colors duration-300 cursor-pointer"
            >
              <Icon36
                name={item.icon}
                className={clsx(
