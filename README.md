...
### 3. 새 작업 브랜치 생성
```bash
git checkout -b KAN-1-fe/feature/event-recommendation/영준
```
...
### 5. 원격 저장소에 푸시
```bash
git push origin KAN-1-fe/feature/event-recommendation/영준
```

### 6. GitHub에서 Pull Request 생성
1. GitHub 저장소 접속
2. **Pull Request** 탭 클릭
3. `KAN-1-fe/feature/event-recommendation/영준` → `dev` 브랜치로 PR 생성
4. 리뷰어 지정 (최소 1명)
5. 코드 리뷰 및 승인 대기
6. 승인 후 merge
...
NEXT_PUBLIC_NCP_CLIENT_SECRET=

NEXT_MAP_SEARCH_CLIENT_ID=
NEXT_MAP_SEARCH_CLIENT_SECRET=
...
# 출발지, 목적지 검색에 필요
NAVER_CLIENT_SEARCH_ID=
NAVER_CLIENT_SEARCH_SECRET=
